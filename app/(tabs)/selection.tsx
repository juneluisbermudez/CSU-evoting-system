...
import { generateMerkleRoot } from '@/lib/generateMerkle';
import { submitMerkleRootToBlockchain } from '@/lib/submitMerkleRoot';
...
export default function CSUVotingApp() {
  ...
  const confirmSubmit = async () => {
    ...
    const merkleRoot = generateMerkleRoot(voteData); // returns hex string
    const txHash = await submitMerkleRootToBlockchain(merkleRoot);

    await supabase.from('vote_roots').insert([
      {
        root: merkleRoot,
        tx_hash: txHash,
